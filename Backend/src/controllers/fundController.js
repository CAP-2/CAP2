...
exports.getCampaigns = async (req, res) => {
    try {
        ...
        // Add actual collected amount to each campaign
...
exports.getTransactions = async (req, res) => {
    try {
        ...
        const [income] = await db.query(`
            SELECT ec.id, ec.amount, ec.contribution_date as date, ec.note, ec.method, 'income' as type, ec.status,
                   p.display_name as person_name, fc.name as campaign_name
        ...
        const [expenses] = await db.query(`
            SELECT ex.id, ex.amount, ex.created_at as date, ex.item_name as note, 'Tiền mặt' as method, 'expense' as type, 'approved' as status,
                   NULL as person_name, fc.name as campaign_name
            FROM event_costs ex
...
exports.addIncome = async (req, res) => {
    try {
        const { amount, date, note, method, person_id, event_id, campaign_id } = req.body;
        const clanId = await getUserClanId(req.user.id);
        if (!clanId) return res.status(403).json({ success: false, message: 'Unauthorized' });

        const [result] = await db.query(
            `INSERT INTO event_contributions (clan_id, event_id, campaign_id, person_id, amount, contribution_date, method, note, status) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'approved')`,
            [clanId, event_id || null, campaign_id || null, person_id || null, amount, date || new Date(), method || 'Tiền mặt', note]
        );
        res.json({ success: true, message: 'Đã thêm khoản thu', id: result.insertId });
    } catch (error) {
...
exports.addExpense = async (req, res) => {
    try {
        const { amount, note, event_id, campaign_id, category } = req.body;
        const clanId = await getUserClanId(req.user.id);
        if (!clanId) return res.status(403).json({ success: false, message: 'Unauthorized' });

        const [result] = await db.query(
            `INSERT INTO event_costs (clan_id, event_id, campaign_id, item_name, amount, note, category) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [clanId, event_id || null, campaign_id || null, note, amount, note, category || 'Khác']
        );
        res.json({ success: true, message: 'Đã thêm khoản chi', id: result.insertId });
    } catch (error) {
...
exports.createCampaign = async (req, res) => {
    try {
        ...
        // BULK NOTIFICATION
        const [members] = await db.query("SELECT a.id FROM accounts a JOIN people p ON a.person_id = p.id WHERE p.clan_id = ?", [clanId]);
        const io = req.app.locals.io;
        const onlineUsers = req.app.locals.onlineUsers;
        for (const m of members) {
            await db.query("INSERT INTO notifications (receiver_account_id, type, title, message) VALUES (?, ?, ?, ?)", 
                [m.id, 'new_campaign', 'Đợt thu mới', `Mở đợt thu: ${name}`]);
            const sid = onlineUsers[m.id];
            if (io && sid) io.to(sid).emit('new_notification', { type: 'new_campaign', title: 'Đợt thu mới', message: `Mở đợt thu: ${name}` });
        }
...
exports.getCampaignDetails = async (req, res) => {
    try {
        const { id } = req.params;
        const [campaignRows] = await db.query('SELECT * FROM fund_campaigns WHERE id = ?', [id]);
        if (!campaignRows.length) return res.status(404).json({ success: false, message: 'Campaign not found' });
        const campaign = campaignRows[0];

        const [transactions] = await db.query(`
            SELECT ec.*, p.display_name as person_name, m.id as media_id
            FROM event_contributions ec
...
exports.reportPayment = async (req, res) => {
    try {
        const { campaign_id, amount, note, method, evidence_media_id } = req.body;
        const [userRows] = await db.query('SELECT person_id FROM accounts WHERE id = ?', [req.user.id]);
        const personId = userRows[0]?.person_id;
        if (!personId) return res.status(400).json({ success: false, message: 'User not linked to person' });

        const [personRows] = await db.query('SELECT clan_id, display_name FROM people WHERE id = ?', [personId]);
        const { clan_id, display_name } = personRows[0];

        await db.query(
            `INSERT INTO event_contributions (clan_id, campaign_id, person_id, amount, contribution_date, method, note, status, evidence_media_id) 
             VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)`,
            [clan_id, campaign_id, personId, amount, new Date(), method || 'Chuyển khoản', note, evidence_media_id || null]
        );

        // Notify managers
        const [managers] = await db.query("SELECT a.id FROM accounts a JOIN people p ON a.person_id = p.id WHERE p.clan_id = ? AND a.role_id = 2", [clan_id]);
        const io = req.app.locals.io;
        for (const m of managers) {
            await db.query("INSERT INTO notifications (receiver_account_id, type, title, message) VALUES (?, ?, ?, ?)", [m.id, 'payment_report', 'Báo cáo nộp quỹ', `${display_name} vừa nộp quỹ.`]);
        }

        res.json({ success: true, message: 'Đã gửi báo cáo thanh toán.' });
    } catch (error) {
...
exports.approvePayment = async (req, res) => {
    try {
        const { transaction_id, status, manager_note } = req.body;
        const clanId = await getUserClanId(req.user.id);
        if (!clanId) return res.status(403).json({ success: false, message: 'Unauthorized' });

        await db.query('UPDATE event_contributions SET status = ?, manager_note = ? WHERE id = ? AND clan_id = ?', [status, manager_note, transaction_id, clanId]);
...
exports.getFundStats = async (req, res) => {
    try {
        ...
        const [incomeByYear] = await db.query("SELECT YEAR(contribution_date) as year, SUM(amount) as total FROM event_contributions WHERE clan_id = ? AND status = 'approved' GROUP BY year ORDER BY year ASC", [clanId]);
        const [expenseByYear] = await db.query("SELECT YEAR(created_at) as year, SUM(amount) as total FROM event_costs WHERE clan_id = ? GROUP BY year ORDER BY year ASC", [clanId]);

        // Category breakdown for current year vs last year
        const curYear = new Date().getFullYear();
        const prevYear = curYear - 1;

        const [categoryStats] = await db.query(`
            SELECT category, YEAR(created_at) as year, SUM(amount) as total
            FROM event_costs
        ...
        res.json({ 
            success: true, 
            yearly: { income: incomeByYear, expense: expenseByYear },
...
exports.exportFundExcel = async (req, res) => {
    try {
        ...
        let incomeQuery = "SELECT ec.contribution_date, p.display_name, ec.amount, ec.method, ec.note, fc.name as campaign FROM event_contributions ec LEFT JOIN people p ON ec.person_id = p.id LEFT JOIN fund_campaigns fc ON ec.campaign_id = fc.id WHERE ec.clan_id = ?";
        let expenseQuery = "SELECT ex.created_at, ex.item_name, ex.amount, ex.category, ex.note FROM event_costs ex WHERE ex.clan_id = ?";
        let params = [clanId];
        ...
        const filename = campaign_id ? `Bao_Cao_Chien_Dich_${campaign_id}.xlsx` : `Bao_Cao_Nam_${year || 'Tat_Ca'}.xlsx`;
...
exports.importFundExcel = async (req, res) => {
    try {
        ...
        for (let row of data) {
            // Basic mapping: Amount, Note, Type (income/expense), Date, Category
            const { amount, note, type, date, category } = row;
            if (type === 'income') {
                await db.query(
                    "INSERT INTO event_contributions (clan_id, amount, contribution_date, note, status, method) VALUES (?, ?, ?, ?, 'approved', 'Excel')",
                    [clanId, amount, date || new Date(), note]
                );
            } else if (type === 'expense') {
                await db.query(
                    "INSERT INTO event_costs (clan_id, amount, created_at, item_name, category) VALUES (?, ?, ?, ?, ?)",
                    [clanId, amount, date || new Date(), note, category || 'Khác']
                );
            }
        }

        res.json({ success: true, message: `Đã nhập thành công ${data.length} dòng dữ liệu` });
    } catch (error) {
...
exports.internalCalculateContributionUnit = async (clanId, definition) => {
    ...
    else if (definition === 'per_family') q = "SELECT COUNT(*) as count FROM families WHERE clan_id = ?";
    else return 0;
